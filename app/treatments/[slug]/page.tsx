...
import { getTreatmentBySlug, getTreatments } from "@/lib/sanity/fetchers";
...
export default async function TreatmentDetailPage({ params }: Props) {
  ...
  // Fetch treatment by slug langsung — lebih efisien dari fetch semua lalu .find()
  const treatment = await getTreatmentBySlug(slug);
  if (!treatment) notFound();

  // Fetch semua untuk related — filter di client
    ...
    <>
      ...
      <section className="border-b border-brand-border bg-cream pt-28 sm:pt-36">
        <div className="mx-auto max-w-7xl px-6 pb-4">
          <div className="flex items-center gap-2 font-inter text-sm text-brand-gray">
            ...
            <span>/</span>
            <span className="text-brand-black line-clamp-1">{treatment.name}</span>
          </div>
        </div>
      </section>
      ...
      <section className="bg-cream py-10 sm:py-16">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 lg:grid-cols-2 lg:gap-16">
          {/* Image */}
          <div className="relative aspect-[4/3] overflow-hidden rounded-brand-xl shadow-luxury bg-cream-200">
            {treatment.image ? (
              <Image
                src={treatment.image}
                alt={treatment.name}
              ...
              <div className="flex h-full w-full items-center justify-center bg-coral-50">
                <span className="font-inter text-sm text-brand-gray">Foto segera hadir</span>
              </div>
            )}
          ...
          <div className="flex flex-col justify-center">
            ...
            {treatment.tagline && (
              <p className="mt-3 font-inter text-lg italic text-coral-600">
                "{treatment.tagline}"
              </p>
            )}
            ...
            {treatment.packages?.length ? (
                  ...
                  {treatment.packages.map((pkg, i) => (
                    ...
                    >
                      ...
                      <span className="font-playfair text-lg font-semibold text-coral-600">
                        {pkg.price ? formatCurrency(pkg.price) : "Hubungi kami"}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="mt-8">
                <p className="font-inter text-sm text-brand-gray">Mulai dari</p>
                <p className="font-playfair text-2xl text-coral-600">{formatCurrency(lowestPrice)}</p>
              </div>
            )}
              ...
              <div>
                ...
                <div className="mt-8 rounded-brand-lg bg-cream p-6">
                  <p className="font-inter text-sm leading-relaxed text-brand-gray">
                    Seluruh kandungan yang digunakan telah melalui uji klinis dan diawasi langsung
                    oleh dokter profesional bersertifikat Melval Labskin.
                  </p>
                </div>
              </div>
            ) : null}
