    ...
    <Card className="group h-full">
      <div className="relative aspect-[3/4] overflow-hidden bg-cream-200">
        ...
        <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-5">
          ...
          <p className="font-inter text-sm text-gold-200">{doctor.specialty}</p>
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-start gap-2">
          ...
          <p className="font-inter text-xs text-brand-gray">{doctor.education[0]}</p>
        </div>
        <div className="mt-2 flex items-start gap-2">
          <BadgeCheck size={16} className="mt-0.5 shrink-0 text-gold-600" />
          <p className="font-inter text-xs text-brand-gray">{doctor.certifications[0]}</p>
        </div>
